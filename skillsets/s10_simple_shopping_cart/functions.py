...
def get_cart(items, costs, tax):
        ...
        # total tax
        print("\nTotal Tax {0:.2f}%: ${1:.2f}".format((tax * 100), (total * tax)))


        # total with tax
        print("\nTotal Amount with Tax: ${0:,.2f}".format(total + (total * tax)))
        print("\n")
        
def get_tax_rate():
    my_tax_rate = 0.0 # initialize tax rate to 0.0

    while True:
        try:
            print("\nEnter tax rate: ", end="")

            my_tax_rate = float(input())

            is_within_range = False
            while not is_within_range:
                if my_tax_rate >= 1 and my_tax_rate <= 10:
                    is_within_range = True
